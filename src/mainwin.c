...
{				
	...
	for (i; i < n; ++i)
	{
	  char* file = image_list_get_current_file_path( param->mw->img_list );
			
	  param->mw->p1 = gdk_pixbuf_new_from_file(file,NULL);
...
{		
  ...
  int n = atoi(b);
  const char* c = g_list_nth_data(list,n); // selected image file path
