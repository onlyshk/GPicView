...
static void start_slideshow(GtkWidget* widget, MainWin* mw);
...
gboolean job_func(GIOSchedulerJob *job, GCancellable *cancellable, gpointer user_data)
{
    ...
    g_io_scheduler_job_send_to_mainloop(param->job, (GSourceFunc)selecting, param->mw, NULL);
...
int selecting(MainWin* mw)
{
	...
	int n = g_list_length (thumbnail_loaded_list);
	...
	image_list_set_current (mw->img_list, g_path_get_basename(g_list_nth_data(thumbnail_loaded_list, c)));
...
{	
	const char* file =  g_file_get_path(mw->loading_file);
...
{
  GList* thumbnail_selected_list = gtk_icon_view_get_selected_items(mw->view); 
...
{ 
    ...
    int image_count = g_list_length (thumbnail_loaded_list);
...
{		
	...
	image_list_set_current (mw->img_list,name);
    ...
    {	
		...
		for (i; i< n; i++)
		{
		   char* paths = g_path_get_basename (g_list_nth_data(thumbnail_loaded_list,i));
...
void on_next( GtkWidget* widget, MainWin* mw )
{
    ...
    if( name )
	{		
        const char* file_path = image_list_get_next( mw->img_list );
...
void
start_slideshow(GtkWidget* widget, MainWin* mw)
{		
	if (mw->slideshow == TRUE)
	{
		...
		g_timeout_add_seconds (0,(GSourceFunc)next_for_slide_show, mw);
	}
	else
	{
